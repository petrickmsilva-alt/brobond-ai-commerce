...
| UI            | **Enterprise dark premium** — glass AppShell · grouped collapsible Sidebar (6 módulos) + workspace switcher · Header com busca global ⌘K, notificações, status TikTok, Nova Campanha e avatar · Glass Dashboard (6 KPIs + 4 gráficos Recharts) · Premium Login (2 colunas) (PR010.1) · **Error Boundaries com digest + Copiar ID · skeletons de loading · empty states com CTA · telas de convite/reset/solicitar acesso** (PR010.2) · **página de confirmação /request-access/success · páginas terminais /invite/invalid e /invite/expired · fila ADMIN de solicitações em /dashboard/settings/access** (PR010.3)                                                                                                                          |
| Currency      | **BRL** (default across Product, Campaign, Sale) · money = integer cents · margin = basis points                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| Auth          | NextAuth v5 (Prisma adapter, JWT) + **Credentials provider (email/senha)** + **Google SSO (opcional — botão oculto quando não configurado)** · **`middleware.ts`** protege as rotas privadas e redireciona para `/login?next=<path>` · convite, reset de senha (TTL 30 min) e solicitação de acesso (PR010.2) · **aprovação cria + envia o convite automaticamente (InvitationMailer/ConsoleMailer) · Google aceita GOOGLE_CLIENT_ID/SECRET ou AUTH_GOOGLE_ID/SECRET · APP_URL para links** (PR010.3)                                                                                                                                                                                                                                        |
| RBAC          | ADMIN > MANAGER > MEMBER — products: ADMIN cria/edita/exclui · MANAGER edita · MEMBER somente leitura · **convites e aprovação de acesso: ADMIN apenas; papel convidável limitado a MANAGER\|MEMBER** (PR010.2) · **aprovação de solicitação provisiona sempre MEMBER; fila dedicada em /dashboard/settings/access** (PR010.3)                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| Tests         | Vitest — **2,276 unit tests** (RBAC, session, tenancy, passwords, pricing, slug, validators, filters, storage, trends, creators, outreach, connectors, matches, campaigns, AI personalization + AI context audit + analytics + TikTok Shop + omnichannel delivery + design-system tokens/contraste AA + navegação + dashboard read model + shell context + **middleware/redirect + sanitização de `next` + convites + reset de senha + visibilidade do Google + error boundary** — OpenAI/Meta/TikTok fully mocked, zero real network calls) (PR010.2) · **orchestration de aprovação · InvitationMailer · roteamento de erros do convite · fluxo de sucesso do request-access · aliases de env do Google · rota ADMIN de acesso** (PR010.3) |
...
| `/request-access`                    | ✅     | Formulário público → `AccessRequest` PENDING (PR010.2) · sucesso navega para a página de confirmação (PR010.3)                                                                                   |
| `/request-access/success`            | ✅     | PR010.3 — confirmação pós-envio; sem formulário; pública                                                                                                                                         |
...
| `/dashboard/settings/access`         | ✅     | PR010.3 §2 — fila ADMIN de solicitações (tabela Nome·Empresa·Email·Telefone·Status, Aprovar/Rejeitar; aprovar emite + envia o convite)                                                           |
...
- `app/dashboard/settings/access/actions.ts` → `reviewAccessRequestAction()`
  (PR010.3 §2 — ADMIN only; APPROVED → `approvalService.approve()` revisa +
  emite + envia o convite MEMBER e devolve o invite URL uma única vez;
  REJECTED → revisa apenas; revalida `/dashboard/settings/access` e
  `/settings`). O painel legado em `/settings` delega a esta ação — as duas
  superfícies nunca divergem.
...
| `tests/auth-access-request.test.ts`                                                                                                                                          | PR010.2 §5 — pedido é _lead_, não conta (sem papel, tenant ou senha), idempotente por email pendente (5 envios → 1 linha), contadores por status, aprovar **não provisiona nada** — o convite explícito continua sendo o único caminho que cria `User`                                                                                                                                                                                                                                                                                                    |

| `tests/auth-approval.test.ts` | PR010.3 §2 — aprovar cria o Invitation (papel **MEMBER**, tenant do revisor, `invitedBy` gravado), só o digest persistido, link entregue pelo mailer com URL do APP_URL, revisão só de PENDING (duplo clique não cria nada), email já cadastrado → `USER_EXISTS` sem alterar a solicitação, falha do mailer não reverte nada (`delivered: false`), rejeitar não provisiona nada |
...
| `tests/auth-request-access-success.test.ts` | PR010.3 §1 — a action grava no banco (payload válido → submit; erro → mensagem genérica sem detalhe), o formulário navega com `router.replace` (nunca `push`), sem estado inline de sucesso, erro permanece na página; rota de confirmação pública e sem formulário |
...
| `tests/auth-settings-access.test.ts` | PR010.3 §2 — actions: `requireAdmin()` primeiro, tenant da sessão (nunca do cliente), APPROVE→orchestrator com invite URL de volta, REJECT→decisão apenas, revalida as duas superfícies; página: MEMBRO/MANAGER veem estado restrito sem ler a fila, ADMIN lê a fila; tabela: colunas Nome·Empresa·Email·Telefone·Status, Aprovar/Rejeitar, ações só para PENDING, coluna oculta sem `canManage`; painel legado delega |
